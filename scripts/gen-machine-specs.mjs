...
function buildErrorCatalog() {
  ...
  return {
    ...
    surfaces: {
      "dig-rpc":
        "JSON-RPC 2.0 codes for the dig RPC (rpc.dig.net). http_or_exit is the transport status (always 200; the error rides in the JSON envelope).",
      "digstore-cli":
        "dig-store CLI process exit codes (0..16). http_or_exit is the process exit code.",
      dighub:
        "DIGHUb (hub.dig.net) user-facing codes. http_or_exit is the HTTP status where one exists.",
      "dig-loader":
        "chia:// content-loader failure codes (DIG Browser / extension). http_or_exit is null (surfaced on the error page).",
    },
    errors: all,
    bySurface,
  };
...
fs.writeFileSync(
  path.join(STATIC, "openrpc-node.json"),
  JSON.stringify(openrpcNode, null, 2) + "\n",
);
