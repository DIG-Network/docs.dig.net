...
export const schemas = {
  ...
  ChunkObject: {
    ...
    properties: {
      ...
      inclusion_proof: {
        ...
        contentEncoding: "base64",
        description:
          "Base64 merkle inclusion proof for the WHOLE resource, relayed verbatim from the capsule. Sent on every window for getContent/getManifest; empty string / null for getCapsule.",
      },
      chunk_lens: {
        type: "array",
        items: { type: "integer", minimum: 0 },
        description:
          "Per-chunk CIPHERTEXT byte lengths of the full resource, in order. REQUIRED to split + decrypt a multi-chunk resource. Emitted on the FIRST window only (offset == 0); empty/absent ⇒ a single chunk = the whole ciphertext.",
      },
      root: {
        ...hex64,
  ...
  MetadataManifest: {
    ...
    type: "object",
    description:
      "The store's plaintext, ungated metadata manifest, embedded in the compiled module (dig-store §8.4).",
    properties: {
  ...
  PeerId: {
    ...
    title: "PeerId",
    description:
      "A DIG Node peer identity: SHA-256 of the peer's TLS SubjectPublicKeyInfo DER, as 64 lower-case hex characters (32 bytes).",
  },
  CandidateAddress: {
    title: "CandidateAddress",
    type: "object",
    description:
      "One address at which a peer may be reachable, tagged by how it was learned. Peers dial candidates most-direct-first.",
    ...
    properties: {
      ...
      kind: {
        ...
        description:
          "How the address was learned: advertised/observed direct, STUN reflexive, UPnP/NAT-PMP/PCP mapped, or relay-reachable.",
      },
    },
  },
  PeerRecord: {
    ...
    type: "object",
    description:
      "A known peer with its identity and candidate addresses (the peer-exchange record).",
    required: ["peer_id", "addresses"],
    properties: {
      peer_id: { $ref: "#/components/schemas/PeerId" },
      addresses: {
        type: "array",
        items: { $ref: "#/components/schemas/CandidateAddress" },
  ...
  AvailabilityAnswer: {
    ...
    properties: {
      ...
      roots: {
        ...
        items: { ...hex64 },
        description:
          "STORE granularity: the generation roots the peer currently holds for the store, newest-first.",
      },
      total_length: {
        type: "integer",
        minimum: 0,
      ...
      complete: {
        type: "boolean",
        description:
          "Whether the peer holds the FULL resource/capsule (true) or only part of it (false); a partial holder still serves the ranges it has.",
      },
    },
  },
  RangeFrame: {
    ...
    properties: {
      offset: {
        ...
        minimum: 0,
        description:
          "This frame's start offset within the requested range (bytes into the resource ciphertext).",
      },
      length: {
        type: "integer",
        minimum: 0,
        description:
          "This frame's byte length (= decoded `bytes` length). At most MAX_RANGE_FRAME_PAYLOAD (32768) — the cap a serve path splits its stream on.",
      },
      bytes: {
        type: "string",
        contentEncoding: "base64",
...
export const methods = [
  {
    ...
    params: [
      { name: "store_id", required: true, schema: { $ref: "#/components/schemas/StoreId" } },
      {
        name: "retrieval_key",
        required: true,
        schema: { $ref: "#/components/schemas/RetrievalKey" },
      },
      ...
      schema: {
        ...
        properties: {
          ...
          execution_proof: {
            type: ["string", "null"],
            description:
              "risc0 receipt or null (read-only / job-based; never a mock receipt on the wire).",
          },
  ...
  },
  {
    ...
    description:
      "Returns the job status and, when terminal, the real risc0 receipt. Never a mock receipt.",
    paramStructure: "by-name",
    params: [
      { name: "store_id", required: true, schema: { $ref: "#/components/schemas/StoreId" } },
  ...
  },
  {
    ...
    description:
      "The node derives the canonical retrieval key for .well-known/dig/manifest.json itself.",
    paramStructure: "by-name",
    params: [
      { name: "store_id", required: true, schema: { $ref: "#/components/schemas/StoreId" } },
      { name: "root", required: false, schema: { $ref: "#/components/schemas/Root" } },
    ...
    result: {
      ...
      schema: {
        allOf: [
          ...
          {
            type: "object",
            properties: { retrieval_key: { $ref: "#/components/schemas/RetrievalKey" } },
          },
        ],
      },
    },
  ...
  },
  {
    ...
    result: {
      ...
      schema: {
        ...
        properties: {
          manifest: {
            oneOf: [{ $ref: "#/components/schemas/MetadataManifest" }, { type: "null" }],
          },
          program_hash: { $ref: "#/components/schemas/StoreId" },
          root: { $ref: "#/components/schemas/StoreId" },
        },
...
export const nodeMethods = [
  ...
  },
  {
    ...
    description:
      "Part of the node-only cache.* control surface (getConfig/setCapBytes/clear/listCached/removeCached/fetchAndCache). NODE-PROFILE ONLY (dig-node lib.rs:1143-1231).",
    paramStructure: "by-name",
    params: [],
  ...
  },
  {
    ...
    result: {
      ...
      schema: {
        ...
        required: ["peers"],
        properties: {
          peers: { type: "array", items: { $ref: "#/components/schemas/PeerRecord" } },
        },
      },
    },
  ...
  },
  {
    ...
    params: [
      ...
      },
      {
        name: "addresses",
        required: true,
        schema: { type: "array", items: { $ref: "#/components/schemas/CandidateAddress" } },
      ...
      schema: {
        ...
        properties: {
          ...
          reachability: {
            ...
            enum: ["direct", "relayed"],
            description:
              "direct = a direct inbound path exists; relayed = only reachable through the relay.",
          },
          relay: {
            type: "object",
    ...
    params: [
      {
        name: "items",
        required: true,
        schema: { type: "array", items: { $ref: "#/components/schemas/AvailabilityItem" } },
    ...
    result: {
      ...
      schema: {
        ...
        properties: {
          items: {
            type: "array",
            items: { $ref: "#/components/schemas/AvailabilityAnswer" },
  ...
  },
  {
    ...
    params: [
      ...
      },
      {
        ...
        description:
          "true to fetch the whole capsule/.dig (identified by store_id[:root]) instead of a resource.",
      },
      {
        name: "offset",
        required: false,
      ...
      },
      {
        ...
        schema: { type: "integer", minimum: 1 },
        description:
          "Bytes to return; clamped to the node window (3 MiB) and widened to whole-chunk boundaries.",
      },
      {
        name: "skip_layout",
        required: false,
        schema: { type: "boolean" },
        description:
          "true when the caller already holds the resource layout + commitment; the peer then omits the layout metadata (chunk_lens, inclusion_proof) and streams data frames only.",
      },
    ],
...
export const rpcErrors = {
  PARSE_ERROR: {
    code: -32700,
    message: "Parse error",
    meaning: "The request body isn’t valid JSON (id is null).",
  },
  INVALID_REQUEST: {
    code: -32600,
    message: "Invalid request",
    meaning: "Not a request object/array, an empty batch, or a missing method.",
  },
  METHOD_NOT_FOUND: {
    code: -32601,
    message: "Method not found",
    meaning: "This node doesn’t implement the method.",
  },
  INVALID_PARAMS: {
    code: -32602,
    message: "Invalid params",
    meaning:
      'Missing/malformed store_id, root, or retrieval_key; or "latest" on a store with no confirmed generation.',
  },
  INTERNAL_ERROR: {
    code: -32603,
    message: "Internal error",
    meaning: "The node failed to satisfy a well-formed call.",
  },
  RESOURCE_UNAVAILABLE: {
    code: -32004,
    message: "Resource not available at the requested root",
    meaning:
      "A genuine infrastructure miss (no host seed, module absent in both buckets, bad magic, oversize, a wasmtime trap, or an undecodable envelope) — distinct from a content miss, which is an indistinguishable decoy with no error.",
  },
  ROOT_NOT_ANCHORED: {
    code: -32005,
  ...
  PEER_UNREACHABLE: {
    ...
    meaning:
      "No connection to the named peer could be established — every NAT-traversal strategy (direct, UPnP/NAT-PMP/PCP mapping, relay-coordinated hole-punch, and relayed fallback) failed, or the peer is not registered on this network. Returned by the node-profile peer methods (dig.getPeers / dig.announce / dig.getNetworkInfo).",
  },
  RANGE_NOT_SATISFIABLE: {
    code: -32007,
    message: "Range not satisfiable",
    meaning:
      "The requested byte range lies outside the resource (offset >= total_length) or is otherwise unsatisfiable. Returned by the node-profile dig.fetchRange when the offset/length cannot be served.",
  },
  RANGE_METADATA_UNREPRESENTABLE: {
    code: -32009,
    message: "Range metadata unrepresentable",
    meaning:
  ...
  UPSTREAM_ERROR: {
    ...
    meaning:
      "A thin-shell node relayed a method it does not resolve locally to its upstream DIG RPC, and the upstream was unreachable or returned a non-JSON response.",
  },
  STAGE_DIR_UNREADABLE: {
    code: -32011,
    message: "Stage: source directory unreadable",
    meaning: "dig.stage (local control) could not read the source directory.",
  },
  STAGE_NO_FILES: {
    code: -32012,
    message: "Stage: no files to stage",
    meaning: "dig.stage found no files in the source directory.",
  },
  STAGE_OVER_CAP: {
    code: -32013,
    message: "Stage: content exceeds capsule size cap",
    meaning: "dig.stage content exceeds the capsule size cap.",
  },
  STAGE_COMPILE_IO: {
    code: -32014,
  ...
  PRIVACY_REQUIRES_LOCAL_NODE: {
    ...
    meaning:
      "mode:\"privacy\" was requested but the caller is not the node's own trusted local originator (loopback / dig.local). A node will not fetch privately on a remote/anonymous caller's behalf (that would hand it the caller's identity + query), so privacy mode requires a local DIG node. See https://docs.dig.net/docs/protocol/onion-routing#toggle-placement.",
  },
  ONION_HOPS_OUT_OF_RANGE: {
    code: -32022,
    message: "Onion hop count out of range",
    meaning:
      "The requested privacy.hops (circuit length) is outside the allowed range [2, 5] (default 3). Request a hop count within range. See https://docs.dig.net/docs/protocol/onion-routing#modes.",
  },
  CONTROL_UNAUTHORIZED: {
    code: -32030,
    message: "Unauthorized (control)",
    meaning:
      "A control.* method was called without a valid local control token. Control methods are loopback-only. Control codes are -32030+ so they never collide with the onion codes -32020/21/22.",
  },
  CONTROL_NOT_SUPPORTED: {
    code: -32031,
    message: "Not supported (control)",
    meaning:
      "A control operation this build/pin cannot perform (e.g. §21 whole-store sync with no loaded identity).",
  },
  CONTROL_ERROR: {
    code: -32032,
    message: "Control error",
    meaning:
      "A control operation failed at runtime — distinct from bad input or an absent capability.",
  },
...
export const cliCatalog = [
  ...
  },
  {
    ...
    description:
      "Not enough XCH or DIG to cover the spend (the uniform capsule price in $DIG + XCH fee per capsule).",
  },
...
export const loaderCatalog = [
  {
    ...
    description:
      "The served ciphertext did not verify against the on-chain generation root (tamper / wrong root).",
  },
  {
