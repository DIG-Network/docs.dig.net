...
You create the store once (in DIGHUb or with [`dig-store init`](../digstore/cli/onchain-anchoring.md), which mints it and spends $DIG). A deploy key never mints and never owns — it only advances an existing store you already control.
:::
...
The issued key goes into CI as the **`DIGSTORE_WRITER_KEY`** secret, and [`dig-store commit`](../digstore/cli/command-reference.md) reads it to sign the root advance.

:::caution `DIGSTORE_WRITER_KEY` + `dig-store commit` — not `dig-store deploy`
The key DIGHUb issues is a **writer** key: it authorizes the on-chain root advance and is consumed by `dig-store commit --deploy-key` (which reads `DIGSTORE_WRITER_KEY`). It is **not** the `dig-store deploy` publisher key (`DIGSTORE_DEPLOY_KEY`) — a §21 head-push credential with no on-chain authority. Put the issued secret in `DIGSTORE_WRITER_KEY`.
:::
...
- name: Publish to DIG
  ...
  run: |
    ...
    dig-store commit --message "ci: $GITHUB_SHA"   # advance the root; signed by the writer key
```
...
```
dig-store authorize-origin-as-writer hub.dig.net
```
