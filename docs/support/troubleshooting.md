...
- **Wrong or missing `salt`** for a **private** store — a private URN needs its `?salt=<hex>`. Pass `--salt <hex>` to `digstore cat`/`checkout`.
...
### "command not found: digstore" {#not-installed}

Open a **new** terminal after installing (so `PATH` refreshes), then `digstore --version`. See [Installing the CLI](../digstore/cli/install.md).
