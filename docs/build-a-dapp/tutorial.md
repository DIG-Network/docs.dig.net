...
- The [`dig-store` CLI](../digstore/cli/install.md) installed.
- Node 18+ and `npm`.
...
`npm create dig-app@latest my-dapp -- --template vite-react` scaffolds the same template straight from npm — the JS front door, no `dig-store` install needed to start. See [Scaffold an app](./scaffold.md) for all five templates and how the two front doors compare.
:::
