...
```sh
digs did create                          # um DID de emissor para atribuição
digs collection create --name "My Drop"  # uma coleção CHIP-0007
digs nft mint --data ./art.png --metadata ./meta.json --dry-run
digs offer make ...                       # trocas de XCH / CAT
```
...
Troque NFTs por XCH ou CATs com `digs offer make | take | show` (cada um compatível com `--dry-run` / `--json`). → [Referência de comandos](../digstore/cli/command-reference.md)
