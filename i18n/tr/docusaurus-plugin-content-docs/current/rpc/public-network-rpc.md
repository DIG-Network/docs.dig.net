...
:::tip
To `clone`, `pull`, or `push` a whole store (not just read resources), use the git-style **[`dig://` remote](./dig-remote.md)** — the same nodes, addressed as `dig://[<user>@]<host>/<storeId>`, with per-request identity-key auth. You can also self-host an origin with `dig-store serve`.
:::
