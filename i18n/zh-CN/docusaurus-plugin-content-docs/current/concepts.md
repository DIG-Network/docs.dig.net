...
capsule 的集合*，每次提交对应一个。这个定义在 DigStore、dig RPC 和 DIG
...
**URN** 将 DigStore 的地址*与*密钥合二为一，写作一个字符串：
...
每个 store 都是 **Chia 主网上的一个 singleton**。`digstore init` 会铸造它（launcher id
*即成为*该 store id），而每次 `digstore commit` 都会作为一次 CHIP-0035 singleton 更新，将新的
...
`digstore` 是用于创建、提交、共享和读取 store 的命令行工具 —— 在加密的链上 store 格式之上，提供了一套
...
`build-command` 及其他项目配置，被 `digstore dev`、`digstore deploy` 和脚手架模板共用。它**不包含任何机密信息**
...
时才需要支付统一的 capsule 价格。它是 Rust CLI 中 `digstore new` 在 npm 一侧的对应工具。→ [搭建应用脚手架](./build-a-dapp/scaffold.md)

## GitHub 部署 Action {#deploy-action}

`dig-network/deploy-action` 是实现 **git-push-to-deploy** 的 GitHub Action：它会在运行器上安装
[`digstore` CLI](#digstore-cli)，运行 `digstore deploy` 来推进你的 store（从不铸造），并将已发布的
...
- [什么是 DigStore？](./digstore/what-is-digstore.md) —— 单文件 store 格式
