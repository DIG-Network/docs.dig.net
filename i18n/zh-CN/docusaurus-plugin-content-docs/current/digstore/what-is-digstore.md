...
与 Git 不同，DigStore 是为**构建产物**而设计的，而不是仓库源码。你将一个项目指向像 `dist/` 这样的目录，它就会捕获其中的内容。
...
- **[DigStore 格式](./format/overview.md)** —— 核心概念：项目、部署、`.wasm` 模块、URN、加密与证明。如果你想了解 DigStore*是什么*，从这里开始。
...
:::note
DigStore 是 [DIG Network](https://dig.net) 的一部分。完整的技术设计参见[协议部分](../protocol-deep-dive.md) —— 内容可寻址的 WASM store 格式。
:::

## 相关链接 {#related}

- [DigStore 格式](./format/overview.md) —— 项目、WASM 模块、URN、加密、证明
