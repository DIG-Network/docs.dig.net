...
```sh
dig-store did create                          # un DID d'émetteur pour l'attribution
dig-store collection create --name "My Drop"  # une collection CHIP-0007
dig-store nft mint --data ./art.png --metadata ./meta.json --dry-run
dig-store offer make ...                       # échanges XCH / CAT
```
...
Échangez des NFT contre du XCH ou des CAT avec `dig-store offer make | take | show` (chacun avec `--dry-run` / `--json`). → [Référence des commandes](../digstore/cli/command-reference.md)
