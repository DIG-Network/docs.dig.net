...
- An existing DIG store (created with `digs init`).
...
- **PRs** run `digs deploy --preview`: a **free**, content-addressed build verified through the real `chia://` read path. The preview address is the `content-address` output and is commented on the PR.
- **Pushes to the default branch** run `digs deploy --if-changed`: a push whose build is byte-identical to the live version is a **no-op** (no spend, nothing published), so it is safe to run on every push.
...
```sh
digs log --json          # copy the "store_id" field (or set it in dig.toml)
```
...
The Action wraps `digs deploy`, which is built for CI. You can run the same flow yourself (e.g. from another CI system):

```sh
digs seed import --mnemonic "$DIG_MNEMONIC"            # DIGSTORE_PASSPHRASE set (funds the fee)
DIGSTORE_WRITER_KEY=<64-hex> digs deploy --output-dir dist --json --if-changed   # advance root + push
digs deploy --preview --output-dir dist --json         # a free preview — no chain, no spend
```

On a fresh checkout `digs deploy` reconstructs the store locally from the deploy key + the current on-chain root, stages your output directory, advances the root (signed by the writer deploy-key), and pushes the new capsule — all non-interactively. See `digs deploy --help`.
