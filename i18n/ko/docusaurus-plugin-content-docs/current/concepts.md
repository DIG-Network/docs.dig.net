...
[store](#store)는 커밋마다 하나씩 생성되는 *capsule들의 시퀀스*입니다. 이 정의는 DigStore, dig RPC,
...
**URN**은 DigStore의 주소이자 키를 하나의 문자열로 결합한 것입니다:
...
모든 store는 **Chia 메인넷 상의 싱글톤**입니다. `digstore init`이 이를 발행하며(launcher id가 store
id가 *됩니다*), 모든 `digstore commit`은 새로운 [generation](#generation) root를 CHIP-0035 싱글톤
...
`digstore`는 store를 생성, 커밋, 공유, 읽기 위한 커맨드라인 도구입니다 — 암호화된 온체인 store
...
`output-dir`, `build-command` 등의 프로젝트 설정을 담고 있으며, `digstore dev`, `digstore deploy`,
...
`digstore new`에 대응하는 npm 측 동반자입니다. → [앱 스캐폴딩하기](./build-a-dapp/scaffold.md)

## GitHub 배포 Action {#deploy-action}

`dig-network/deploy-action`은 **git-push-to-deploy** 방식의 GitHub Action입니다: 러너에
[`digstore` CLI](#digstore-cli)를 설치하고, `digstore deploy`를 실행해 store를 진전시키며(mint는
...
- [DigStore란?](./digstore/what-is-digstore.md) — 단일 파일 store 포맷
