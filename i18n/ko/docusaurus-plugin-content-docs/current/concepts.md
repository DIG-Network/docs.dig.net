...
모든 store는 **Chia 메인넷 상의 싱글톤**입니다. `digs init`이 이를 발행하며(launcher id가 store
id가 *됩니다*), 모든 `digs commit`은 새로운 [generation](#generation) root를 CHIP-0035 싱글톤
...
`output-dir`, `build-command` 등의 프로젝트 설정을 담고 있으며, `digs dev`, `digs deploy`,
...
`digs new`에 대응하는 npm 측 동반자입니다. → [앱 스캐폴딩하기](./build-a-dapp/scaffold.md)

## GitHub 배포 Action {#deploy-action}

`dig-network/deploy-action`은 **git-push-to-deploy** 방식의 GitHub Action입니다: 러너에
[`dig-store` CLI](#digstore-cli)를 설치하고, `digs deploy`를 실행해 store를 진전시키며(mint는
