...
`digs new`は、実行可能でウォレット対応済みのプロジェクトを書き出します。Reactテンプレートを選びましょう。
