...
```sh
digstore did create                          # eine Issuer-DID für die Zuordnung
digstore collection create --name "My Drop"  # eine CHIP-0007-Kollektion
...
Handle NFTs gegen XCH oder CATs mit `digstore offer make | take | show` (jeweils `--dry-run` / `--json`). → [Befehlsreferenz](../digstore/cli/command-reference.md)
