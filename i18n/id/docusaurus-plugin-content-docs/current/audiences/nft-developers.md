...
```sh
digs did create                          # sebuah DID penerbit untuk atribusi
digs collection create --name "My Drop"  # sebuah koleksi CHIP-0007
digs nft mint --data ./art.png --metadata ./meta.json --dry-run
digs offer make ...                       # perdagangan XCH / CAT
```
...
Perdagangkan NFT untuk XCH atau CAT dengan `digs offer make | take | show` (masing-masing dengan `--dry-run` / `--json`). → [Referensi perintah](../digstore/cli/command-reference.md)
