...
|---|---|---|---|
| **push** | `b"dig-store:push:v1"` | `SHA-256(PUSH_DST \|\| root(32) \|\| store_id(32))` → 32B | `bls.rs:194-200` |
| **request** | `b"dig-store:req:v1"` | `SHA-256(REQ_DST \|\| u32be(len(method)) \|\| method \|\| store_id(32) \|\| u64be(ts) \|\| nonce(32))` → 32B | `bls.rs:267-281` |
| **node** | `b"dig-store:node:v1"` | `NODE_DST \|\| program_hash(32) \|\| public_output(32) \|\| chia_header_hash(32) \|\| u32be(height) \|\| public_input` (**variable**, NOT pre-hashed) | `bls.rs:209-224` |
| **attest** | `b"dig-store:attest:v1"` | `ATTEST_DST \|\| nonce(32) \|\| store_id(32) \|\| u64be(ts)` (**variable**) | `bls.rs:232-243` |
| **tomb** | `b"dig-store:tomb:v1"` | `SHA-256(TOMB_DST \|\| canonical(Tombstone))` → 32B | `bls.rs:250-256` |
...
`NODE_DST = b"dig-store:node:v1"` is the same byte string as the [merkle `NODE_TAG`](./merkle-proofs.md#domain-separation). The two are domain-distinct in practice because they are consumed in disjoint preimage shapes: a BLS node-attestation message versus a fixed 65-byte merkle pair.
:::
