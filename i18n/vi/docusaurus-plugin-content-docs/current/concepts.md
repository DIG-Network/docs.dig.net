...
Mỗi store là một **singleton trên Chia mainnet**. `dig-store init` mint nó (launcher id *trở thành* store id) và
mỗi `dig-store commit` neo một root [generation](#generation) mới on-chain dưới dạng một bản cập nhật singleton
...
`build-command`, và các cấu hình dự án khác, được chia sẻ bởi `dig-store dev`, `dig-store deploy`, và các template
...
capsule đồng nhất khi xuất bản một [capsule](#capsule). Đây là đối tác phía npm của lệnh `dig-store new` trong CLI
...
trên runner, chạy `dig-store deploy` để tiến store của bạn thêm một bước (không bao giờ mint), và báo cáo
