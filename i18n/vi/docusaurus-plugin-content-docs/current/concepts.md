...
các capsule*, mỗi capsule ứng với một lần commit. Định nghĩa này giống hệt nhau trên DigStore, dig RPC, và DIG
...
Một **URN** là địa chỉ *và* khóa của DigStore gộp trong một chuỗi:
...
Mỗi store là một **singleton trên Chia mainnet**. `digstore init` mint nó (launcher id *trở thành* store id) và
mỗi `digstore commit` neo một root [generation](#generation) mới on-chain dưới dạng một bản cập nhật singleton
...
`digstore` là công cụ dòng lệnh tạo, commit, chia sẻ, và đọc các store — một luồng công việc theo phong cách Git
...
`build-command`, và các cấu hình dự án khác, được chia sẻ bởi `digstore dev`, `digstore deploy`, và các template
...
capsule đồng nhất khi xuất bản một [capsule](#capsule). Đây là đối tác phía npm của lệnh `digstore new` trong CLI
Rust. → [Dựng khung một ứng dụng](./build-a-dapp/scaffold.md)

## GitHub deploy Action {#deploy-action}

`dig-network/deploy-action` là GitHub Action **git-push-to-deploy**: nó cài đặt [CLI `digstore`](#digstore-cli)
trên runner, chạy `digstore deploy` để tiến store của bạn thêm một bước (không bao giờ mint), và báo cáo
...
- [DigStore là gì?](./digstore/what-is-digstore.md) — định dạng store một-file
